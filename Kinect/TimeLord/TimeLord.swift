...
extension Date {
    //TODO: This is the format that Dermot required
    public static var toStringFormat: String{
       return "yyyy-MM-dd HH:mm:ss:SSS ZZZ"
    }
    ...
    public func toDateTimeStatsportsString(_ inTimeZone: TimeZone? = nil) -> String{
        return self.toStringInFormat(Date.toStringFormat, inTimeZone: inTimeZone)
    }
    ...
    public func getHourMinForLocalTimeZone() -> String{
        let format = DateFormatter()
        format.timeZone = TimeZone.autoupdatingCurrent
        
        format.dateFormat = TimeZone.autoupdatingCurrent.identifier.contains("US") ? "h:mm a" : "HH:mm"
    ...
    public func diffInMiliseconds(_ fromDate: Date, absoluteValue: Bool = true) -> Int{
        let miliseconds = (Calendar.current as NSCalendar).components(NSCalendar.Unit.nanosecond, from: self, to: fromDate, options: []).nanosecond! / 1000000
        return absoluteValue ? abs(miliseconds) : miliseconds
    }
